...
| How is the SQL layer unit tested? | Through a thin `SqlDriver` interface (`select`, `execute`) with two implementations: `pluginSqlDriver.ts` (real app) and `testDriver.ts` (`node:sqlite`'s built-in `DatabaseSync`, in-memory, vitest-only). Repo functions depend on the interface, never a concrete driver — mirrors how Rust repo functions took `&Connection`. Amended during plan-writing: `better-sqlite3` (the option named during brainstorming) failed to compile on this machine (no matching MSVC ClangCL toolset); `node:sqlite`, built into the Node 24 runtime already in use, needed no native build and was verified working (numbered `?1` placeholders, multi-statement `exec`, `PRAGMA foreign_keys` cascade behavior all confirmed by hand before committing to the plan). |
